...
public class Environment {	
		...
		public static Environment getInstanceBySession(Map<String, Object> session){
			System.out.println(session);
			if (session.containsKey("environment") == true){
				return (Environment)session.get("environment");
			}
		...
		public Environment(String userAgent){
			...
			//Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/29.0.1547.76 Safari/537.36
				...
				browserName = "Chrome";
				browserVersion = browserName + " " + userAgent.substring( userAgent.indexOf('/', index ) + 1, userAgent.indexOf('.', index) + 2 );
				if( osPlatform.equals("Mobile") == false ){
					osPlatform = "Desktop";
				}
			}
