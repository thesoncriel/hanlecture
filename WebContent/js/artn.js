...
//			this.registBreakEnter();
	...
	createDialog : function(strSelector){
		...
		$( sSelector ).each(function(index){
			...
			var bAutoOpen = (jqDialog.data("auto-open") === "true");
			var sModal = "true";
			var isNonTitle = (jqDialog.attr("title") === undefined);
			
			if ( jqDialog.data("modal") ){
				sModal = jqDialog.data("modal");
			}
...
Artn.BreadCrumbs = {
		...
		ARROW_TAGS : " <span class=\"arrow\">&gt;</span> ",
		HOME_TAGS : "<span class=\"artn-icon-16 home\">⊙</span><a href=\"/\">HOME</a>",
		SELECTOR_BC : "#breadcrumbs",
		SELECTOR_SIDE_NAV : ".side-nav",
		SELECTOR_GNB : ".gnb",
		
		_option : null,
		_bcAnchors : {},
		
		create : function(mOption){
			//TODO: 옵션에 따라 다르게 적용 시킬 내용 필요. - 2014.01.07 by jhson
			if (mOption !== undefined) return this;
			
			var sContentsCode = "";
			var jqListAnchors = null;
			
			this._option = {
				arrowTags: 	this.ARROW_TAGS,
				jqBC: 		$( this.SELECTOR_BC ),
				jqSideNav: 	$( this.SELECTOR_SIDE_NAV ),
				targetNav:	this.SELECTOR_SIDE_NAV.replace( ".", "" )
			};
			
			sContentsCode = this.extractContentsCodeByKeys( "contents", "sub" );
			
			if (sContentsCode !== null){
				if ( sContentsCode.indexOf("*") === 0 ){
					this.appendCustomMenu( sContentsCode.replace( "*", "" ) );
				}
				else{
					jqListAnchors = this.markSelected( this._option.jqSideNav, this._option.targetNav, sContentsCode );
					this.appendMenu( jqListAnchors );
				}
			}
		...
		extractContentsCodeByKeys : function( sKey1, sKey2 ){
			var sContentsCode = this._option.jqBC.data( sKey1 );
			
			if (sContentsCode === undefined){
				sContentsCode = this._option.jqBC.data( sKey2 );
			}
			
			if (sContentsCode === undefined){
				console.log( sKey1 + "과 " + sKey2 + "와 같은 키네임으로 데이터를 가져오는데 실패 하였습니다." );
		...
		markSelected : function(jqTarget, sNavClass, sContentsCode){
			var jqMenuItem = jqTarget.find( "." + sContentsCode );
			
			this._bcAnchors[ sNavClass ] = [];
			
			if ( jqMenuItem.length > 0 ){
				jqMenuItem = jqMenuItem.eq( jqMenuItem.length - 1 );
				this._bcAnchors[ sNavClass ][ 0 ] = jqMenuItem.clone();
				jqMenuItem.addClass( "selected" );
				this.markParentSelected( jqMenuItem, sNavClass );
			}
			else{
				console.log( sContentsCode + "라는 컨텐츠 코드로 관련 노드를 찾을 수가 없습니다." );
			}
			
			return this._bcAnchors[ sNavClass ];
		},
		
		markParentSelected : function( jqMenuItem, sNavClass ){
			var jqParent = jqMenuItem.parent();
			var jqSiblingAnchor = null;
			var jqH3 = null;
			var sTagName = jqParent.prop( "tagName" ).toLowerCase();
			
			if ( jqParent.hasClass( sNavClass ) === true ){
				jqH3 = jqParent.children( "h3" );
				
				if ( jqH3.length > 0 ){
					jqSiblingAnchor = jqH3.find("a");
					
					if ( jqSiblingAnchor.length > 0 ){
						this._bcAnchors[ sNavClass ][ this._bcAnchors[ sNavClass ].length ] = this.removeTagWithBeyondBR( jqSiblingAnchor.clone() );
					}
				}
				...
				this._bcAnchors[ sNavClass ][ this._bcAnchors[ sNavClass ].length ] = this.removeTagWithBeyondBR( jqParent.clone() );
				jqParent.addClass( "selected" );
			}
			else if ( sTagName === "ul" ){
				jqSiblingAnchor = jqParent.siblings("a");
				if ( jqSiblingAnchor.length > 0 ){
					if ( jqSiblingAnchor.parent().hasClass( sNavClass ) === false ){
						jqSiblingAnchor.addClass( "selected" );
						this._bcAnchors[ sNavClass ][ this._bcAnchors[ sNavClass ].length ] = this.removeTagWithBeyondBR( jqSiblingAnchor.clone() );
					}
				}
			...
			this.markParentSelected( jqParent, sNavClass );
		},
		
		appendMenu : function( jqListAnchors ){
			var iLen = jqListAnchors.length;
			
			this._option.jqBC.append( this.HOME_TAGS );
			
			for (var i = iLen - 1; i >= 0; i--){
				this._option.jqBC
				.append( this._option.arrowTags )
				.append( jqListAnchors[ i ] );
			}
		},
		
		appendCustomMenu : function( sText ){
			this._option.jqBC
			.append( "<span class=\"artn-icon-16 home\">⊙</span><a href=\"/\">HOME</a>" )
			.append( this._option.arrowTags )
			.append( "<a href=\"#\">" + sText + "</a>" );
		},
